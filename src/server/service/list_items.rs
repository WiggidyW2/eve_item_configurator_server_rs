use super::super::{accessors::Accessor, data::DivisionNames, error::Error, pb, typedef::TypeId};
use super::{
    list_items_procedure::{Iterate, Keep, Procedure},
    service::Service,
};
    ...
    pub async fn list_items_authorized(
        ...
        match procedure.iterate {
            ...
            Iterate::Items => set_list_items(
                ...
                items
                    .into_iter()
                    .map(|(type_id, json_idx)| (type_id, true, json_idx)),
            ),
            Iterate::Types => set_list_items(
                ...
                types.into_iter().rev().map(|type_id| {
                    let (json_idx, enabled) = match (items.remove(&type_id), req.include_json) {
